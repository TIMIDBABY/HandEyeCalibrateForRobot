...
class UR_Robot_Enhanced:
    ...
    def move_c(self, via_pose, to_pose, speed=speed, acceleration=acc):
        ...
        def _move_c():
            print(
                f"🌀 圆弧运动: 经过 {[f'{pos:.3f}' for pos in via_pose[:3]]} 到达 {[f'{pos:.3f}' for pos in to_pose[:3]]}"
            )
    ...
    def move_to_position(
        ...
        def _move_to_position():
            ...
            changes = []
            if x is not None:
                changes.append(f"x={x:.3f}")
            if y is not None:
                changes.append(f"y={y:.3f}")
            if z is not None:
                changes.append(f"z={z:.3f}")
            if rx is not None:
                changes.append(f"rx={np.rad2deg(rx):.2f}°")
            if ry is not None:
                changes.append(f"ry={np.rad2deg(ry):.2f}°")
            if rz is not None:
                changes.append(f"rz={np.rad2deg(rz):.2f}°")
