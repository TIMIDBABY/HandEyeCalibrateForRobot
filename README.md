...
在手眼标定结束后可进入Acc_test文件夹，根据项目需求，修改hand_in_main.py或者hand_out_main.py中的各项数据，进行标定结果的验证；其中**main.py**仅适用于UR5与D435相机结合的**眼在手外**项目，如果需要测试自己的项目，请自行替换相机文件（realsenseD435.py）与机器人文件（arm_control.py）。
